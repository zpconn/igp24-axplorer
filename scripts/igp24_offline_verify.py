...
def build_pari_input(records: list[dict[str, Any]]) -> str:
    candidate_rows = [
        f"  [\"{record['canonical_hash']}\", {json.dumps(record['exported_coefficients'])}]"
    ...
    for index, row in enumerate(candidate_rows):
        suffix = "," if index < len(candidate_rows) - 1 else ""
...
def parse_online_magma_pasted_output(
    ...
    *,
    ...
    return {
        ...
        "computed_r": parsed.get("signature_r"),
        "exact_r_source": "magma_number_of_real_roots" if parsed.get("signature_r") is not None else None,
        "exact_r_status": "ok" if parsed.get("signature_r") is not None else "missing",
...
def _parse_marker_int(line: str, marker: str) -> int | None:
    payload = _marker_payload(line, marker)
    if payload is None:
        return None
    numbers = re.findall(r"-?\d+", payload.replace(" ", ""))
    if not numbers:
        numbers = re.findall(r"-?\d+", payload)
    return int(numbers[-1]) if numbers else None


def _parse_marker_bool(line: str, marker: str) -> bool | None:
    payload = _marker_payload(line, marker)
    if payload is None:
        return None
    lowered = payload.lower()
    if lowered in {"1", "true", "yes"} or "true" in lowered:
        return True
    if lowered in {"0", "false", "no"} or "false" in lowered:
        return False
    return None


def _finalize_pari_record(record: dict[str, Any], *, source_path: Path | None) -> dict[str, Any]:
    nfdisc = record.get("nfdisc_abs")
    nfdisc_status = record.get("nfdisc_status")
    if nfdisc is not None:
        status = "nfdisc_ok"
        nfdisc_status = nfdisc_status or "ok"
    elif nfdisc_status == "error":
        status = "nfdisc_error"
    else:
        status = "parse_error"
        nfdisc_status = nfdisc_status or "missing"
    record.update(
        {
            "schema_version": 1,
            ...
            "parsed_at": datetime.now(timezone.utc).isoformat(),
            "status": status,
            "nfdisc_status": nfdisc_status,
            "exact_nfdisc_status": "ok" if nfdisc is not None else nfdisc_status,
            "exact_nfdisc_abs": nfdisc,
            "field_disc_abs": nfdisc,
            "nfdisc_source": "pari_gp_nfdisc" if nfdisc is not None else None,
            "raw_output_source_path": str(source_path) if source_path else None,
            "provenance": {
                "tool": "PARI/GP",
                "workflow": "nfdisc",
                "manual_or_local_output": True,
            },
            "safety": {
                "sair_submission": False,
                "network_calls_by_helper": False,
...
def parse_pari_output(text: str, *, source_path: Path | None = None) -> list[dict[str, Any]]:
    """Parse machine-readable markers emitted by build_pari_input."""
    ...
    for raw_line in text.splitlines():
        ...
        if not line:
            continue
        payload = _marker_payload(line, "IGP24_PARI_BEGIN")
        if payload is not None:
            if current is not None:
                records.append(_finalize_pari_record(current, source_path=source_path))
            current = {"candidate_hash": payload.split()[0] if payload else None}
            continue
        if current is None:
            continue
        if "IGP24_PARI_END" in line:
            records.append(_finalize_pari_record(current, source_path=source_path))
            current = None
            continue
        degree = _parse_marker_int(line, "IGP24_PARI_DEGREE")
        if degree is not None:
            current["pari_degree"] = degree
            current["degree"] = degree
            continue
        irreducible = _parse_marker_bool(line, "IGP24_PARI_IS_IRREDUCIBLE")
        if irreducible is not None:
            current["pari_is_irreducible"] = irreducible
            continue
        real_roots = _parse_marker_int(line, "IGP24_PARI_REAL_ROOT_COUNT")
        if real_roots is not None:
            current["pari_real_root_count"] = real_roots
            continue
        poly_disc = _parse_marker_int(line, "IGP24_POLDISC_ABS")
        if poly_disc is not None:
            current["poly_disc_abs"] = abs(poly_disc)
            current["polynomial_discriminant_abs"] = abs(poly_disc)
            continue
        nfdisc = _parse_marker_int(line, "IGP24_NFDISC_ABS")
        if nfdisc is not None:
            current["nfdisc_abs"] = abs(nfdisc)
            continue
        status_payload = _marker_payload(line, "IGP24_NFDISC_STATUS")
        if status_payload is not None:
            current["nfdisc_status"] = status_payload.split()[0].lower() if status_payload else "missing"
            continue
        error_payload = _marker_payload(line, "IGP24_NFDISC_ERROR")
        if error_payload is not None:
            current["nfdisc_error"] = error_payload
            current.setdefault("nfdisc_status", "error")
            continue
    if current is not None:
        records.append(_finalize_pari_record(current, source_path=source_path))
    return records
...
def load_pari_pasted_outputs(paths: list[Path] | None) -> list[dict[str, Any]]:
    """Load saved PARI/GP output files and parse nfdisc result markers."""

    rows: list[dict[str, Any]] = []
    for path in paths or []:
        source_path = path.resolve()
        rows.extend(parse_pari_output(source_path.read_text(encoding="utf-8"), source_path=source_path))
    return rows


def build_pari_summary(
    *,
    records: list[dict[str, Any]],
    results: list[dict[str, Any]],
    output_dir: Path,
    ...
    return {
        "schema_version": 1,
        "record_type": "igp24_pari_nfdisc_summary",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "tool": "scripts/igp24_offline_verify.py",
        "source_commit": source_commit,
        "command": command,
        ...
        "execution": execution,
        "output_files": {
            "pari_results_jsonl": str(output_dir / PARI_RESULTS_JSONL),
            "pari_summary_json": str(output_dir / PARI_SUMMARY_JSON),
            "pari_report_md": str(output_dir / PARI_REPORT_MD),
            "pari_input_gp": str(output_dir / PARI_INPUT_GP),
            "pari_raw_output": str(output_dir / PARI_RAW_OUTPUT),
        },
        "safety": {
            ...
            "note": "PARI/GP nfdisc artifacts are local/manual evidence only and never submit to SAIR.",
        },
    }


def build_pari_report(summary: dict[str, Any], results: list[dict[str, Any]]) -> str:
    lines = [
        "# IGP24 PARI/GP nfdisc Report",
        "",
        "PARI/GP nfdisc artifacts are local/manual evidence only and never submit to SAIR.",
        "",
        f"- Input: `{summary.get('input_path')}`",
        f"- Input kind: `{summary.get('input_kind')}`",
        f"- Records loaded: {summary.get('records_loaded')}",
        f"- Parsed result records: {summary.get('parsed_result_records')}",
        f"- Exact nfdisc records: {summary.get('nfdisc_records')}",
        f"- Status counts: `{json.dumps(summary.get('status_counts', {}), sort_keys=True)}`",
        f"- PARI executed: `{(summary.get('execution') or {}).get('executed')}`",
        f"- Saved-output paths: `{json.dumps(summary.get('pasted_output_paths') or [])}`",
        "",
        "| status | hash | degree | irreducible | r(pari) | nfdisc | poly_disc |",
        "| --- | --- | ---: | --- | ---: | ---: | ---: |",
    ]
    if not results:
        lines.append("|  |  |  |  |  |  |  |")
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    str(result.get("pari_is_irreducible") if result.get("pari_is_irreducible") is not None else ""),
                    str(result.get("pari_real_root_count") if result.get("pari_real_root_count") is not None else ""),
    ...
    lines.extend(
        [
            "",
            "Artifacts:",
            f"- Results JSONL: `{summary.get('output_files', {}).get('pari_results_jsonl')}`",
            f"- Summary JSON: `{summary.get('output_files', {}).get('pari_summary_json')}`",
            f"- Input GP script: `{summary.get('output_files', {}).get('pari_input_gp')}`",
            "",
        ]
    )
    return "\n".join(lines)
...
def write_pari_artifacts(
    *,
    summary: dict[str, Any],
    results: list[dict[str, Any]],
    output_dir: Path,
) -> dict[str, Path]:
    results_path = output_dir / PARI_RESULTS_JSONL
    summary_path = output_dir / PARI_SUMMARY_JSON
    report_path = output_dir / PARI_REPORT_MD
    _write_jsonl(results_path, results)
    summary_path.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    report_path.write_text(build_pari_report(summary, results), encoding="utf-8")
    return {
        "pari_results_jsonl": results_path,
        "pari_summary_json": summary_path,
        "pari_report_md": report_path,
    }
...
def write_outputs(
    *,
    ...
    magma_common_search_patterns: list[str] | None = None,
    pari_pasted_outputs: list[Path] | None = None,
    ...
    pari_results: list[dict[str, Any]] = []
    pari_raw_path = execution["pari"].get("raw_output_path")
    if pari_raw_path:
        pari_results.extend(parse_pari_output(Path(str(pari_raw_path)).read_text(encoding="utf-8"), source_path=Path(str(pari_raw_path))))
    pari_results.extend(load_pari_pasted_outputs(pari_pasted_outputs))
    pari_summary = build_pari_summary(
        records=records,
        results=pari_results,
        output_dir=output_dir,
        input_path=input_path,
        input_kind=input_kind,
        command=command,
        source_commit=source_commit,
        execution=execution["pari"],
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=[],
        help="Saved PARI/GP output file containing IGP24_PARI/IGP24_NFDISC markers to parse",
    )
    parser.add_argument(
        "--online_magma_manual",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    paths = write_outputs(
        ...
        pari_pasted_outputs=[path.resolve() for path in args.pari_pasted_output or []],

...
def build_plan(
    *,
    ...
    sync_status_payload = sync_status or {"partial_sync": False, "submission_state_complete": True}
    partial_sync = bool(sync_status_payload.get("partial_sync"))
    submission_state_complete = bool(sync_status_payload.get("submission_state_complete", not partial_sync))
    ...
    return {
        ...
        "inputs": {
            ...
            "sync_partial": partial_sync,
            "sync_submission_state_complete": submission_state_complete,
            "sync_failing_endpoint": sync_status_payload.get("failing_endpoint"),
            "sync_error_code": sync_status_payload.get("error_code"),
        ...
        "decision": {
            ...
            "submission_reason": (
                "partial SAIR sync: submission/scoring state is incomplete, so no submission packet can be recommended"
                if partial_sync or not submission_state_complete
                else "no candidate packet was generated by this planning helper; any future packet must pass score-aware and anti-basin gates plus SAIR dry-run"
            ),
            "full_sync_required_before_submission": bool(partial_sync or not submission_state_complete),
...
def build_markdown(plan: dict[str, Any]) -> str:
    lines = [
        ...
        f"- Partial sync: `{plan['inputs'].get('sync_partial')}`",

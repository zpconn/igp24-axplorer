...
from scripts.igp24_offline_verify import (
    ...
    SYMPY_SIGNATURE_RESULTS_JSONL,
    SYMPY_SIGNATURE_SUMMARY_JSON,
    ...
    compute_sympy_signature_result,
...
def test_build_magma_verification_input_command_and_parser(tmp_path):
    ...
    assert "IGP24_SIGNATURE" in online_script
...
<calculator><headers><max_time>60</max_time><max_input>50000</max_input><seed>52682226</seed><version>2.29-8</version><time>0.420</time><memory>32.09MB</memory></headers><results><line>IGP24_BEGIN 70a542863f79ad17cf1a61789241eae078e6984669278e551f7015795d2f03cb</line><line>IGP24_DEGREE 24</line><line>IGP24_IS_IRREDUCIBLE true</line><line>IGP24_SIGNATURE 4</line><line>IGP24_GALOIS_GROUP Symmetric group G acting on a set of cardinality 24</line><line>Order = 2^22 * 3^10 * 5^4 * 7^3 * 11^2 * 13 * 17 * 19 * 23</line><line>IGP24_TRANSITIVE_GROUP_ID 25000</line><line>IGP24_END 70a542863f79ad17cf1a61789241eae078e6984669278e551f7015795d2f03cb</line></results></calculator>"""
    ...
    assert parsed["signature_r"] == 4
    assert parsed["computed_r"] == 4
    assert parsed["exact_r_source"] == "magma_number_of_real_roots"
    assert parsed["exact_r_status"] == "ok"
...
def test_parse_pari_output_records_nfdisc_and_poly_disc():
    output = "\n".join(
        [
            "IGP24_PARI_BEGIN abc",
            "IGP24_PARI_DEGREE 24",
            "IGP24_PARI_IS_IRREDUCIBLE 1",
            "IGP24_PARI_REAL_ROOT_COUNT 4",
            "IGP24_POLDISC_ABS 999",
            "IGP24_NFDISC_ABS 123",
            "IGP24_NFDISC_STATUS ok",
            "IGP24_PARI_END abc",
        ]
    )

    parsed = parse_pari_output(output)

    assert parsed == [
        {
            "candidate_hash": "abc",
            "degree": 24,
            "exact_nfdisc_abs": 123,
            "exact_nfdisc_status": "ok",
            "field_disc_abs": 123,
            "nfdisc_abs": 123,
            "nfdisc_source": "pari_gp_nfdisc",
            "nfdisc_status": "ok",
            "parsed_at": parsed[0]["parsed_at"],
            "pari_degree": 24,
            "pari_is_irreducible": True,
            "pari_real_root_count": 4,
            "poly_disc_abs": 999,
            "polynomial_discriminant_abs": 999,
            "provenance": {
                "manual_or_local_output": True,
                "tool": "PARI/GP",
                "workflow": "nfdisc",
            },
            "raw_output_source_path": None,
...
def test_compute_sympy_signature_result_records_exact_real_root_count():
    parsed = compute_sympy_signature_result(_record("signature_hash", -5), index=1)

    assert parsed["status"] == "signature_ok"
    assert parsed["exact_r_source"] == "sympy_poly_count_roots"
    assert parsed["exact_r_status"] == "ok"
    assert parsed["degree"] == 24
    assert parsed["sympy_real_root_count"] == 2
    assert parsed["signature_r"] == 2
    assert parsed["computed_r"] == 2
    assert parsed["provenance"]["magma_signature_workflow"] is False
    assert parsed["safety"]["network_calls"] is False
...
def test_write_outputs_creates_manifest_scripts_plan_and_safety_flags(tmp_path):
    ...
    assert (output_dir / SYMPY_SIGNATURE_RESULTS_JSONL).exists()
    ...
    sympy_signature_summary = json.loads((output_dir / SYMPY_SIGNATURE_SUMMARY_JSON).read_text(encoding="utf-8"))
    ...
    assert manifest["output_files"]["sympy_signature_results_jsonl"] == str(output_dir / SYMPY_SIGNATURE_RESULTS_JSONL)
    ...
    assert sympy_signature_summary["requested"] is False
    assert sympy_signature_summary["status_counts"] == {}
    assert sympy_signature_summary["signature_records"] == 0
    ...
    assert not manifest["safety"]["sympy_signature_executed"]
    assert not manifest["safety"]["sympy_signature_results_parsed"]
...
def test_write_outputs_can_run_explicit_sympy_signature(tmp_path):
    review_dir = tmp_path / "review"
    _write_review_batch(review_dir, [_record("signature_hash", -5)])
    loaded, loaded_dir, source_manifest = load_review_batch(review_dir)
    output_dir = tmp_path / "offline"
    ...
    paths = write_outputs(
        ...
        source_review_manifest=source_manifest,
        output_dir=output_dir,
        command=["python3", "scripts/igp24_offline_verify.py", "--run_sympy_signature"],
        source_commit="abc123",
        pari_executable="definitely_missing_gp",
        magma_executable="definitely_missing_magma",
        run_pari=False,
        run_magma=False,
        run_sympy_signature=True,
        timeout_seconds=5,
        magma_common_search_patterns=[],
    )

    manifest = json.loads(paths["offline_verification_manifest_json"].read_text(encoding="utf-8"))
    sympy_summary = json.loads(paths["sympy_signature_summary_json"].read_text(encoding="utf-8"))
    sympy_results = [
        json.loads(line)
        for line in paths["sympy_signature_results_jsonl"].read_text(encoding="utf-8").splitlines()
    ]

    assert sympy_summary["requested"] is True
    assert sympy_summary["status_counts"] == {"signature_ok": 1}
    assert sympy_summary["signature_records"] == 1
    assert sympy_results[0]["exact_r_status"] == "ok"
    assert sympy_results[0]["sympy_real_root_count"] == 2
    assert sympy_results[0]["exact_r_source"] == "sympy_poly_count_roots"
    assert manifest["safety"]["sympy_signature_executed"] is True
    assert manifest["safety"]["sympy_signature_results_parsed"] is True
    assert manifest["safety"]["exact_r_claims"] is True
...
def test_write_outputs_creates_online_magma_manual_artifacts_and_parses_paste(tmp_path):
    ...
    loaded, loaded_dir, source_manifest = load_review_batch(review_dir)
    pasted_xml = """<?xml version=\"1.0\"?>
<calculator><headers><max_time>60</max_time><max_input>50000</max_input><seed>52682226</seed><version>2.29-8</version><time>0.420</time><memory>32.09MB</memory></headers><results><line>IGP24_BEGIN 70a542863f79ad17cf1a61789241eae078e6984669278e551f7015795d2f03cb</line><line>IGP24_DEGREE 24</line><line>IGP24_IS_IRREDUCIBLE true</line><line>IGP24_SIGNATURE 4</line><line>IGP24_GALOIS_GROUP Symmetric group G acting on a set of cardinality 24</line><line>IGP24_TRANSITIVE_GROUP_ID 25000</line><line>IGP24_END 70a542863f79ad17cf1a61789241eae078e6984669278e551f7015795d2f03cb</line></results></calculator>"""
    ...
    assert "IGP24_SIGNATURE" in generated_script
    ...
    assert results[0]["signature_r"] == 4

...
def test_build_sample_export_diversity_commands_are_bounded_export_only(tmp_path):
    ...
    assert strategies["fixed_template_t10_top12"] == "fixed_sparse_template"
    ...
    assert by_variant["fixed_template_t10_top12"]["caps"]["temperature"] == 1.0
    assert by_variant["fixed_template_t10_top12"]["caps"]["top_k"] == 12
...
def test_build_sample_export_target_r_conditioned_command_accepts_diversity_sampling_knobs(tmp_path):
    ...
    assert config["caps"]["sample_export_required_support_patterns"] == "sparse_mixed_support_gcd1"
    ...
    assert command[command.index("--sample_export_required_support_patterns") + 1] == "sparse_mixed_support_gcd1"
